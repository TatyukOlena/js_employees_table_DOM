...
thead.addEventListener("click", (even)=>{
    ...
    if (even.target.tagName !== "TH") return;
    const index = Array.from(th).indexOf(target);
    const rowsArr = Array.from(document.querySelector("tbody").rows);
    if (index === lastSortedColumnIndex) sortDirection = sortDirection === "asc" ? "desc" : "asc";
    ...
    rowsArr.sort((a, b)=>{
        ...
        if (!isNaN(Number(aClean)) && !isNaN(Number(bClean))) {
            if (sortDirection === "asc") return aClean - bClean;
            else return bClean - aClean;
        } else {
            if (sortDirection === "asc") return aClean.localeCompare(bClean);
            else return bClean.localeCompare(aClean);
        }
    });
...
form.addEventListener("submit", (sendForm)=>{
    ...
    if (valueName.length >= 4 && valueAge >= 18 && valueAge <= 90 && valueSalary !== undefined && valuePosition !== undefined && valueOffice !== undefined) {
        ...
        pushNotification("success", "Employee added", "The new employee was added to the table.");
        tr.addEventListener("click", function() {
            document.querySelectorAll("tbody tr").forEach((r)=>r.classList.remove("active"));
            this.classList.add("active");
        });
    } else pushNotification("error", "Error", "Please check the form fields");
});
